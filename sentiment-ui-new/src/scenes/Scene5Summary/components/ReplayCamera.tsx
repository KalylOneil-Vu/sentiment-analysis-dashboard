  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-3 left-3 right-3 flex items-center gap-2">
        ...
        />
        <span className="text-[10px] font-medium tracking-wide text-slate-700">
          <span className="text-slate-500">REPLAYING:</span>{' '}
...
function drawFaceOverlay(
  ...
  if (leftEyePoints.length > 0) {
    ...
    ctx.beginPath()
    ctx.arc(transformX(leftEyeCenter.x), transformY(leftEyeCenter.y), 12, 0, Math.PI * 2)
    ctx.stroke()
  }
  ...
  if (rightEyePoints.length > 0) {
    ...
    ctx.beginPath()
    ctx.arc(transformX(rightEyeCenter.x), transformY(rightEyeCenter.y), 12, 0, Math.PI * 2)
    ctx.stroke()
  }
