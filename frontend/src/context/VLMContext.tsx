  ...
  const loadModel = useCallback(
    async (onProgress?: (msg: string) => void) => {
      ...
      loadPromiseRef.current = (async () => {
        try {
          onProgress?.("Loading FastVLM processor...");
          processorRef.current = await AutoProcessor.from_pretrained(MODEL_ID);

          onProgress?.("Processor loaded. Loading FastVLM model (this may take 1-2 minutes)...");
          ...
          onProgress?.("FastVLM model loaded successfully!");
          setIsLoaded(true);
        } catch (e) {
