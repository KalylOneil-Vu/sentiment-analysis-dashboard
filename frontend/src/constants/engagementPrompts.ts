 ...
 */
export function extractKeywords(vlmResponse: string): {
  positive: string[];
  neutral: string[];
  negative: string[];
  bodyLanguage: string[];
 ...
 */
export function calculateEngagementFromKeywords(keywords: ReturnType<typeof extractKeywords>): number {
  ...
  if (keywords.bodyLanguage.some(kw => ENGAGEMENT_KEYWORDS.BODY_LANGUAGE.OPEN.includes(kw))) {
    score += 0.15;  // Increased from 0.1
  }
  if (keywords.bodyLanguage.some(kw => ENGAGEMENT_KEYWORDS.BODY_LANGUAGE.CLOSED.includes(kw))) {
    score -= 0.15;
  }
  if (keywords.bodyLanguage.some(kw => ENGAGEMENT_KEYWORDS.BODY_LANGUAGE.ACTIVE.includes(kw))) {
    score += 0.15;  // Increased from 0.1
  }

  // Emotion adjustments (stronger impact + count multiple emotions)
  const positiveEmotionCount = keywords.emotions.filter(em =>
    ENGAGEMENT_KEYWORDS.EMOTIONS.POSITIVE.includes(em)
  ).length;
  const negativeEmotionCount = keywords.emotions.filter(em =>
    ENGAGEMENT_KEYWORDS.EMOTIONS.NEGATIVE.includes(em)
  ).length;

  if (positiveEmotionCount > 0) {
    score += Math.min(0.25, positiveEmotionCount * 0.08);  // More emotions = higher boost
  }
